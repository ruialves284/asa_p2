...
int EdmondsKarp(Node *adjList, int **F);
int BFS(Node *adjList, int **F, int P[], int M[]);
void bfs2(Node *adjList, int **F);

void initQueue(int size);
void enqueue(int v);
int dequeue();

void printResMatrix(int **F);

void printAdjList(Node *adjList);
void checkMaxFlow(Node *adjList);
void printImage(Node *adjList);
void freeList(Node *adjList, int **F);
...
int main() {
  ...
  int **F = malloc((numNodes+1) * sizeof(int *));
  for (i = 0; i <= numNodes; i++) {
    F[i] = malloc((numNodes+1) * sizeof(int));
  }
